...
class PcCommandsBot(PcCommands):
    ...
    async def processor_info(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
         ...
         await update.message.reply_text(f"Архитектура вашего процессора: {machine_processor}")
         await update.message.reply_text(f'Кол во потоков: {psutil.cpu_count()}')
         await update.message.reply_text(f'Кол во ядер: {psutil.cpu_count(logical=False)}')
