...
чем могу помочь?:"""
    ...
    async def answer_question(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
        user_text = update.message.text
        user_text_clean = user_text.lower().strip()
        
        to_do_list = ["sport", "programming", "college", "codding 40 min"]
        
        
        if 'как зовут моих двух котиков?' in user_text_clean:
            await update.message.reply_text("Барсик и Пушок")
            return True
        
        if "выведи мой список дел на сегодня" in user_text_clean:
            tasks = "\n".join(f"{task}" for task in to_do_list)
            await update.message.reply_text(f"Вот ваш список дел на сегодня -> \n{tasks}")
            return True
                
        if "сколько сейчас времени?" in user_text_clean:
            from datetime import datetime
            time_now = datetime.now().strftime("%H:%M:%S")
            await update.message.reply_text(f"Сейчас: {time_now} часов")
            return True
