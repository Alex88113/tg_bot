...
class CommandsBot(ABC):
    ...
    @abstractmethod
    async def info_about_system(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        pass
...
/help: Выводит всю информацию об доступных командах
...
чем могу помочь?:"""
    ...
    async def info_about_system(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text("Время работы процессора: {}".format(psutil.cpu_times())) # type: ignore
        await update.message.reply_text(f"Кол - во логических процессоров вашего пк составляет: {psutil.cpu_count(logical=True)}") # type: ignore
